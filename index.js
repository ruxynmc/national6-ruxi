...
const myId = "rcoman";
const taskList = [];
const task = document.getElementById("input-new-task");
...
function renderToDoList(taskList) {
    for (const taskContent of taskList) {
        renderList(taskContent);
    }
}

function renderList(taskContent) {
    ...
    removeTask.innerHTML = trashSvg;

    currentTask.appendChild(checkbox);
    currentTask.appendChild(newTask);
    currentTask.appendChild(removeTask);
    ...
    removeTask.addEventListener("click", () => {
        currentTask.remove();
        fetch(
            `https://simple-json-server-scit.herokuapp.com/todo/${myId}`,
            {
                method: "DELETE",
            }).then(getData);
    });
