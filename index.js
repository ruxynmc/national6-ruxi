...
addButton.addEventListener("click", sendTaskCotent);
...
function renderToDoList(response) {
    ...
    for (const element of response.todo) {
        ...
        deleteTask.innerHTML = trashSvg;
        deleteTask.classList.add("delete-button-svg");
        ...
        newTask.innerText = element.item;
        inputNewTask.value = "";
...
function sendTaskCotent() {
    if(payload.todo === undefined){
        payload.todo = [
            {
                checked: false,
                item: inputNewTask.value
            }
        ];
...
function checkTask(selectedTask) {
    const task = selectedTask.nextSibling.innerText;
    if(selectedTask.checked) {
        for(const element of payload.todo) {
            if(task === element.item) {
...
function removeTask(selectedTask) {
    const task = selectedTask.previousSibling.innerText;
    for (const element of payload.todo) {
        if(task === element.item) {
            const taskIndex = payload.todo.indexOf(element);
            selectedTask.parentElement.remove();
            payload.todo.splice(taskIndex, 1);
